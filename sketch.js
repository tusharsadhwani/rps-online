    ...
    case Status.WAITING:
      async function play_hand(hand) {
        const res = await fetch(
          `${url}/play?room=${room_code}&id=${user_id}&hand=${hand}&round=${round}`
        );
...
function setup_logo() {
  if (width > height) {
    ...
    if (logo_landscape.width > width) logo_landscape.resize(width, 0);
  } else {
...
function add_text_field(text_field_var, x, field_name) {
  ...
  if ((x == 0.2 && room_code_selected) || (x == 0.5 && user_name_selected)) {
    fill(255, 108, 23, 200);
  }
  rect(
    width * 0.1,
    container_top + container_height * x,
    text_box_width,
    text_box_height
  );
  ...
  text(
    text_field_var,
    width * 0.11,
    container_top + container_height * (x - 0.005),
    text_box_width,
    text_box_height
  );
}
...
function add_rng_button() {
  ...
  strokeWeight(4);
  rect(
    width * 0.11 + text_box_width + 4,
    container_top + container_height * 0.5,
    text_box_height,
    text_box_height
  );
  image(
    generate_img,
    width * 0.11 + text_box_width + text_box_height / 2 + 4,
    container_top + container_height * 0.5 + text_box_height / 2,
    text_box_height * 0.7,
    text_box_height * 0.7
  );
}
...
function show_welcome_screen() {
  ...
  rect(
    width / 2,
    container_top + container_height * 0.4,
    btn_width,
    btn_height
  );
  fill(0);
  ...
  rect(
    width / 2,
    container_top + container_height * 0.6,
    btn_width,
    btn_height
  );
  fill(0);
...
function show_newgame_screen() {
  ...
  rect(
    width / 2,
    container_top + container_height * 0.8,
    btn_width,
    btn_height
  );
...
function show_waiting_screen() {
  ...
  if (height / width < 1.3) {
    ...
    text(
      "Chosen Hand:",
      width / 2,
      container_top + container_height - image_height
    );
    if (chosen_hand !== null) {
      let hand_img = get_left_img(chosen_hand);
      image(
        hand_img,
        width / 2,
        container_top + container_height - image_height / 2,
        image_width * 0.8,
        image_height * 0.8
      );
    }
...
function show_hands() {
  ...
  for (let i = 0; i < people_right; i++) {
    ...
    right_y.push(
      container_top + ((i + 0.5 * offset) / people_left) * container_height
    );
  }
  ...
  for (let i = 0; i < right_y.length; i++) {
    ...
    image(
      hand,
      width,
      y - image_height / 2 - y_offset,
      -image_width,
      image_height
    );
    text(
      players[people_left + i].name,
      width - image_width / 2,
      y + image_height / 2
    );
  }
...
function randomHand() {
  ...
  if (tmp < 1 / 3) return ROCK;
  else if (tmp < 2 / 3) return PAPER;
